...
function Menu({ visibility, toggleVisibility }) {
  const buttons = [
    <Button key="Work" color="success">
      Work
    </Button>,
    <Button key="Home">Home</Button>,
    <Button key="Leisure" color="#4caf50">
      Leisure
    </Button>,
  ];
        ...
        <>
          ...
          <ButtonGroup orientation="vertical">
            {buttons.map((button, index) => (
              <Button
                key={index}
                className={`categoryButton_${index}`}
                onClick={() => {
...
function TaskCreator(buttonType) {
  ...
  const [taskData, setTaskData] = useState({
    ...
    size: 0,
    deadline: null,
    category: buttonType.buttonType.toLowerCase(),
    completed: false,
  });
      ...
      <div className="datePicker">
        <LocalizationProvider dateAdapter={AdapterDayjs} adapterLocale="en-gb">
          <MobileDatePicker
            ...
            onChange={(newValue) =>
              setTaskData((prev) => {
                const date = dateConverter(newValue.format("DD/MM/YYYY"));
